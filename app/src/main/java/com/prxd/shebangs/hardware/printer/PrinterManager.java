package com.prxd.shebangs.hardware.printer;
...
import com.prxd.shebangs.common.PrintContent;
import com.prxd.shebangs.hardware.printer.bluetooth.BluePrinterDevice;
import com.prxd.shebangs.tool.Utils;
import com.prxd.shebangs.ui.cashier.cash.SaleGoodsInformation;
import com.prxd.shebangs.hardware.printer.bluetooth.DeviceConnFactoryManager;
import com.prxd.shebangs.hardware.printer.bluetooth.PrinterCommand;
import com.prxd.shebangs.hardware.printer.bluetooth.ThreadPool;
...
import static com.prxd.shebangs.hardware.printer.PrinterDevice.LINK_TYPE_BLUETOOTH;
import static com.prxd.shebangs.hardware.printer.bluetooth.DeviceConnFactoryManager.ACTION_QUERY_PRINTER_STATE;
import static com.prxd.shebangs.hardware.printer.bluetooth.DeviceConnFactoryManager.CONN_STATE_FAILED;
...
public class PrinterManager {
    ...
    private void registerPrinterInterfaceListener() {
        ...
        SheBangsApp.getInstance().registerReceiver(this.mLinkPrinterReceiver, filter2);
    }
    ...
    public void scanPrinter(int type) {
        switch (type) {
            case PrinterDevice.LINK_TYPE_BLUETOOTH:
    ...
    public void printSaleBill(SaleGoodsInformation information) throws IOException {
        ...
        if (escManager != null) {
            ...
            throw new IOException(SheBangsApp.getInstance().getString(R.string.str_bill_printer_no_conn));
        }
    }
    ...
    public void printSaleLabel(final String label) throws IOException {
        ...
        if (tscManager != null) {
            ...
            throw new IOException(SheBangsApp.getInstance().getString(R.string.str_code_printer_no_conn));
        }
    }
...
//                    break;
    ...
    public void unManager() {
        SheBangsApp.getInstance().unregisterReceiver(mLinkPrinterReceiver);
    ...
    private class BlueToothPrinterBusiness extends PrinterBusiness {
        ...
        private BlueToothPrinterBusiness() {
            ...
            //初始化蓝牙设备组--这个是内部使用的
            super.bluePrinterGroup = new PrinterDeviceGroup(SheBangsApp.getInstance().getString(R.string.printer_bluetooth_title), PrinterDevice.LINK_TYPE_BLUETOOTH);
            ...
            SheBangsApp.getInstance().registerReceiver(this.mFindBlueToothReceiver, filter);
        }

        @Override
        protected void initialization() {
            SharedPreferences preferences = SheBangsApp.getInstance().getSharedPreferences(BT_PB, Context.MODE_PRIVATE);
            ...
            for (Map.Entry<String, String> set : devices.entrySet()) {
                ...
                PrinterDevice bDev = new BluePrinterDevice(PrinterDevice.LINK_TYPE_BLUETOOTH, Integer.parseInt(attr[0]), set.getKey(), attr[1]);
        ...
        protected void close() {
            SheBangsApp.getInstance().unregisterReceiver(mFindBlueToothReceiver);
        ...
        private final BroadcastReceiver mFindBlueToothReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                if (BluetoothDevice.ACTION_FOUND.equals(action)) {
                    ...
                    Utils.toast(SheBangsApp.getInstance(), SheBangsApp.getInstance().getString(R.string.printer_scan_finish));
